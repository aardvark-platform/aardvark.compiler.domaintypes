...
//    module Methods =
//                                
//        let apply self reuseCache originalId fields newFields tName = 
...
//
//        let toMod self reuseCache originalId fields newFields tmName =
//            let svData =
//                let flags =
//                    {
//                        IsInstance = true
...
//
//                        let apply = Methods.apply self reuseCache originalId fields newFields tName
//                        let toMod = Methods.toMod self reuseCache originalId fields newFields tmName
//                        let iUniqueImpl = Methods.iUniqueImpl self
//
//
...
module Preprocessing =
    ...
    let generateMutableModels (l : list<FSharpEntity>) =
        for e in l do
            printfn "namespace %s" (FSharpEntity.mutableNameSpace e)
            printfn "open System"
            printfn "open Aardvark.Base.Incremental"
            printfn ""
            if e.IsFSharpRecord then
                let fields = e.FSharpFields |> Seq.map (fun f -> FSharpType.isDomainType f.FieldType, f) |> Seq.toList
                ...
                printfn ""
                printfn "type %s(__initial : %s) =" mutableName immutableName
                printfn "    let mutable __current = __initial"

                for (isDomain, f) in fields do
                    let fName = f.Name
                    if isDomain then
                        let mName = FSharpType.mutableName f.FieldType
                        printfn "    let _%s = %s(__initial.%s)" fName mName fName
                    else
                        printfn "    let _%s = %s(__initial.%s)" fName resetType fName
                ...
                printfn "    member x.Apply(__model : %s) =" immutableName
                printfn "        if not (Object.ReferenceEquals(__model, __current)) then"
                printfn "            __current <- __model"
                
                for (isDomain, f) in fields do
                    let fName = f.Name
                    if isDomain then
                        printfn "            _%s.Apply(__model.%s)" fName fName
                    else
                        printfn "            _%s.Update(__model.%s)" fName fName
    ...
    let runInternal (fsProjPath : string) =
        async {
            ...
            if res.HasCriticalErrors then
                ...
                let code = domainTypes |> Map.map (fun _ c -> generateMutableModels c)
                return Some code
        }
